...
class UserRegistrationFormTests(TestCase):
    ...
    def test_birth_date_below_minimum_age(self):
        form_data = {
            'first_name': 'John',
            'last_name': 'Doe',
            'email': 'john@example.com',
            ...
            'account_type': self.bank_account_type.pk,
            'gender': 'M',
            'birth_date': (date.today() - timedelta(days=365 * 17)).isoformat()  # 17 years old
        }
        form = UserRegistrationForm(data=form_data)
        self.assertFalse(form.is_valid(), msg=form.errors)
        self.assertIn('birth_date', form.errors)
...
class LoginViewTests(TestCase):
    ...
    def test_redirection_for_authenticated_user(self):
        ...
        response = self.client.get(reverse('accounts:user_login'))
        self.assertEqual(response.status_code, 302)  # Adjust as needed for post-login redirection

    # def test_non_field_error_display(self):
    #     # Test non-field errors by providing invalid credentials
    #     response = self.client.post(reverse('accounts:user_login'), {
    #         'email': 'invaliduser@iiitt.ac.in',
    #         'password': 'wrongpassword'
    #     })
    #     self.assertEqual(response.status_code, 200)
    ...
    def test_successful_login_redirection(self):
        # Check successful login redirects to the dashboard
        response = self.client.post(reverse('accounts:user_login'), {
            'email': self.email,
            'password': self.password
        })
